...
namespace GPUInstancer.CrowdAnimations
{
    public class GPUICrowdShaderBindings : GPUInstancerShaderBindingsExtension
    {
        ...
        {
            return GetInstancedShader(shaderInstances, shaderName, null);
        }

        public Shader GetInstancedShader(List<ShaderInstance> shaderInstances, string shaderName, string[] keywords = null)
        {
            ...
            if (GPUInstancerConstants.gpuiSettings.isURP && shaderName == GPUICrowdConstants.SHADER_UNITY_URP_LIT)
            {
#if UNITY_2020_2_OR_NEWER
                if (keywords != null && keywords.Contains("_SPECULAR_SETUP") && Shader.Find(GPUICrowdConstants.SHADER_GPUI_CROWD_URP_LIT_SPECULAR) != null)
                    return Shader.Find(GPUICrowdConstants.SHADER_GPUI_CROWD_URP_LIT_SPECULAR);
#endif
                if (Shader.Find(GPUICrowdConstants.SHADER_GPUI_CROWD_URP_LIT) != null)
        ...
        public Material GetInstancedMaterial(List<ShaderInstance> shaderInstances, Material originalMaterial)
        {
            ...
            Material instancedMaterial = new Material(GetInstancedShader(shaderInstances, originalMaterial.shader.name, originalMaterial.shaderKeywords));
