...
namespace GPUInstancer.CrowdAnimations
{
    public class GPUICrowdAnimator : GPUIAnimatorWorkflow
    {
        ...
        private static readonly Vector4 DEFAULT_WEIGHT = new Vector4(1.0f, 0f, 0f, 0f);
        public void StartAnimation(GPUICrowdRuntimeData runtimeData, int arrayIndex, GPUIAnimationClipData clipData, float startTime = -1.0f, float speed = 1.0f, float transitionTime = 0)
        {
            ...
            if (previousClipIndex == 0 && startTime == -1.0f && speed == currentClipSpeeds[0] && currentAnimationClipDataWeights == DEFAULT_WEIGHT) // Unnecessary StartAnimation call
                return;

            if (transitionIndex >= 0) // Already in transition
                CompleteTransition(runtimeData);
            ...
            if (hasTransition)
            {
                ...
                Vector3 normalizedWeights = new Vector3(currentAnimationClipDataWeights.x, currentAnimationClipDataWeights.y, currentAnimationClipDataWeights.z).normalized;
                currentAnimationClipDataWeights.w = normalizedWeights.z;
                currentAnimationClipDataWeights.z = normalizedWeights.y;
                currentAnimationClipDataWeights.y = normalizedWeights.x;
                currentAnimationClipDataWeights.x = 0.01f;
                currentAnimationClipDataWeights = Vector4.Normalize(currentAnimationClipDataWeights);
                //Debug.Log(currentAnimationClipDataWeights);
                ...
                transition.SetData(arrayIndex, currentTime, transitionTime, activeClipCount, currentAnimationClipDataWeights, DEFAULT_WEIGHT, 1);
        ...
        public void CompleteTransition(GPUICrowdRuntimeData runtimeData)
        {
        ...
        public void ApplyAnimationEvents(GPUICrowdRuntimeData runtimeData, GPUICrowdPrefab crowdInstance, float currentTime, float deltaTime)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (runtimeData.eventDict.TryGetValue(clipData.clipIndex, out gpuiAnimationEvents))
                {
                    ...
                    //    continue;
                    bool isLoopDisabled = clipData.IsLoopDisabled();
                    float currentClipFrame = GetClipFrame(i, currentTime, clipData.length, clipData.clipFrameCount, isLoopDisabled);
                    float previousClipFrame = GetClipFrame(i, currentTime - deltaTime, clipData.length, clipData.clipFrameCount, isLoopDisabled);
                    if (currentClipFrame == previousClipFrame)
                        continue;
                    //Debug.LogWarning("currentClipFrame " + currentClipFrame + " previousClipFrame" + previousClipFrame);
                    foreach (GPUIAnimationEvent gpuiAnimationEvent in gpuiAnimationEvents)
                    {
                        if ((gpuiAnimationEvent.eventFrame < currentClipFrame && (gpuiAnimationEvent.eventFrame >= previousClipFrame || previousClipFrame > currentClipFrame))
                            || (!isLoopDisabled && currentClipFrame < previousClipFrame && previousClipFrame < gpuiAnimationEvent.eventFrame) // For looping animations that started a new loop.
                            || (isLoopDisabled && gpuiAnimationEvent.eventFrame == currentClipFrame && gpuiAnimationEvent.eventFrame == clipData.clipFrameCount - 1)) // For non-looping animations with an event at the last frame.
