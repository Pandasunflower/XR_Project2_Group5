...
namespace GPUInstancer.CrowdAnimations
{
    ...
    public class GPUICrowdManager : GPUInstancerPrefabManager
    {
        ...
        {
            _lastUpdateTime = Time.time - Time.deltaTime;
                            ...
                            foreach (GPUISkinnedMeshData smd in crowdPrototype.animationData.skinnedMeshDataList)
                            {
                                if (smd.isOptional && !smd.isOptionalPrototypeGenerated)
                                {
                                    foreach (SkinnedMeshRenderer skinnedMeshRenderer in skinnedMeshRenderers)
                                    {
                                        ...
                                        {
                                            GPUICrowdPrototype generatedPrototype = DefineGameObjectAsCrowdPrototypeAtRuntime(skinnedMeshRenderer.gameObject, crowdPrototype.animationData, false, crowdPrototype);
        ...
        public override void Update()
        {
            ...
            float currentDeltaTime = currentTime - _lastUpdateTime;
        ...
        public override void SetRenderersEnabled(GPUInstancerPrefab prefabInstance, bool enabled)
        {
            ...
            if (crowdPrefabInstance.animatorRef != null)
            {
                if (!enabled && prototype.animationData.IsOptimizeGameObjects())
                    AnimatorUtility.OptimizeTransformHierarchy(crowdPrefabInstance.animatorRef.gameObject, prototype.animationData.useCrowdAnimator && !prototype.animationData.applyBoneUpdates ? null : prototype.animationData.exposedTransforms);
