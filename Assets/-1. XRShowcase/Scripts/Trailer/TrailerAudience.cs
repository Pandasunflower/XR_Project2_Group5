...
public class TrailerAudience : MonoBehaviour
{
    [Header("References")]
    public GPUICrowdManager gpuiCrowdManager;
    public List<GameObject> connectAudience;
    ...
    public void GenerateCrowd()
    {
        ...
        StartCoroutine(SetCrowdAnimators(0));
    }

    public IEnumerator ShowAudienceCoroutine()
    {
        List<GameObject> pool = new List<GameObject>(connectAudience);

        while (pool.Count > 0)
        {
            int randomIndex = Random.Range(0, pool.Count);

            GameObject audience = pool[randomIndex];

            pool.RemoveAt(randomIndex);

            StartCoroutine(ScaleUp(audience.transform));
    ...
    IEnumerator SetWaveCrowdAnimators(int index)
    {
        if (gpuiCrowdManager != null)
        {
            while (!gpuiCrowdManager.isInitialized)
