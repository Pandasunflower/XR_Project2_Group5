...
namespace GPUInstancer
{
    public class DetailDemoSceneController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // Setup space ship
            _spaceShip = GPUInstancerUtility.FindObjectOfType<SpaceshipController>().transform;
            _spaceShipThrusterGlow = _spaceShip.GetChild(0).GetChild(0).GetComponent<ParticleSystem>();

            // Setup grass mower
            _grassMower = GPUInstancerUtility.FindObjectOfType<GrassMowerController>().transform;
