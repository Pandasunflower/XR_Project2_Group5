    ...
    public class GPUInstancerPrefabManager : GPUInstancerManager
    {
        ...
        public virtual GPUInstancerRuntimeData InitializeRuntimeDataForPrefabPrototype(GPUInstancerPrefabPrototype p, int additionalBufferSize = 0)
        {
            ...
            if (runtimeData == null)
            {
                ...
                if (p.isShadowCasting)
                {
                    ...
                    {
                        runtimeData.shadowCasterMaterial = new Material(GPUInstancerUtility.FindShader(GPUInstancerConstants.SHADER_GPUI_SHADOWS_ONLY));
                    }
                }
            ...
            else if (_registeredPrefabsRuntimeData.TryGetValue(p, out registeredPrefabsList))
            {
                ...
                if (runtimeData.bufferSize > 0)
                {
                    ...
                    if (hasAutoTransformUpdate && runtimeData.bufferSize > 0)
                    {
                        ...
                        Transform t = runtimeData.prototype.prefabObject.transform;
                        for (int i = 0; i < runtimeData.bufferSize; i++)
                            runtimeData.instanceTransformAccessArray.Add(t);
#else
        ...
        public static void ExpandBufferSize(GPUInstancerRuntimeData runtimeData, int newBufferSize)
        {
            if (runtimeData.bufferSize < newBufferSize)
            {
                ...
                {
                    runtimeData.instanceTransformAccessArray = GPUInstancerUtility.ResizeTransformAccessArray(runtimeData.instanceTransformAccessArray, newBufferSize, runtimeData.prototype.prefabObject.transform);
                }
        ...
        public virtual void RegisterPrefabsInScene()
        {
            ...
            GPUInstancerPrefab[] scenePrefabInstances = GPUInstancerUtility.FindObjectsOfType<GPUInstancerPrefab>();
