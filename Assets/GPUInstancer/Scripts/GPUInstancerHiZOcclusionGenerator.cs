...
namespace GPUInstancer
{
    public class GPUInstancerHiZOcclusionGenerator : MonoBehaviour
    {
...
#if GPUIPRO
        public bool hasProCamera;
        public GPUInstancerPro.GPUICamera proCamera;
#endif
        ...
        private void Awake()
        {
            hiZTextureSize = new Vector4(0, 0, 1, 1);
        ...
        {
#if GPUIPRO
            hasProCamera = TryGetComponent(out proCamera);
            if (hasProCamera) return;
#endif
            _isInvalid = false;
        ...
        public bool IsHiZDepthUpdated()
        {
            if (hiZDepthTexture == null)
                return false;
#if GPUIPRO
            if (hasProCamera && proCamera != null) 
                return proCamera.GetCameraData().OcclusionCullingData.IsHiZDepthUpdated;
#endif
            return true;
        }
        ...
        {
#if GPUIPRO
            if (!hasProCamera && TryGetComponent(out proCamera))
                Initialize();
            ...
            if (hasProCamera)
            {
                ...
                {
                    hiZDepthTexture = proCamera.GetCameraData().HiZDepthTexture;
                    if (hiZDepthTexture != null)
                    {
                        Vector2Int proTextureSize = proCamera.GetCameraData().HiZTextureSize;
                        hiZTextureSize = new Vector4(proTextureSize.x, proTextureSize.y, hiZDepthTexture.width, hiZDepthTexture.height);
                    }
                }
                return;
            }
...
#endif
            return new Vector4(screenSize.x, screenSize.y, screenSize.x, screenSize.y);
        }
