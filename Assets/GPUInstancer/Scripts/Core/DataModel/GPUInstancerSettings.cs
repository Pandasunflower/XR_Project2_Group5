...
namespace GPUInstancer
{
    public class GPUInstancerSettings : ScriptableObject
    {
        ...
        public bool loadFromAddressables;
        ...
        public Material GetFoliageHDRPTemplate()
        {
            ...
            _foliageHDRPTemplate = GPUInstancerUtility.LoadResource<Material>(GPUInstancerConstants.FOLIAGE_SHADER_HDRP_TEMPLATE_MATERIAL_PATH);
        ...
        {
            GPUInstancerSettings gpuiSettings = GPUInstancerUtility.LoadResource<GPUInstancerSettings>(GPUInstancerConstants.SETTINGS_PATH + GPUInstancerConstants.GPUI_SETTINGS_DEFAULT_NAME);
...
#endif
                shaderBindings = GetDefaultGPUInstancerShaderBindings(!loadFromAddressables);
            }
        }

        private static GPUInstancerShaderBindings GetDefaultGPUInstancerShaderBindings(bool createNewIfNull = true)
        {
            GPUInstancerShaderBindings shaderBindings = GPUInstancerUtility.LoadResource<GPUInstancerShaderBindings>(GPUInstancerConstants.SETTINGS_PATH + GPUInstancerConstants.SHADER_BINDINGS_DEFAULT_NAME);

            if (createNewIfNull && shaderBindings == null)
            {
...
#endif
                billboardAtlasBindings = GetDefaultGPUInstancerBillboardAtlasBindings(!loadFromAddressables);
            }
        }

        private static GPUInstancerBillboardAtlasBindings GetDefaultGPUInstancerBillboardAtlasBindings(bool createNewIfNull = true)
        {
            GPUInstancerBillboardAtlasBindings billboardAtlasBindings = GPUInstancerUtility.LoadResource<GPUInstancerBillboardAtlasBindings>(GPUInstancerConstants.SETTINGS_PATH + GPUInstancerConstants.BILLBOARD_ATLAS_BINDINGS_DEFAULT_NAME);

            if (createNewIfNull && billboardAtlasBindings == null)
            {
        ...
        public virtual void SetDefaultShaderVariantCollection()
        {
            ...
            if (shaderVariantCollection == null)
            {
                ...
                shaderVariantCollection = GetDefaultShaderVariantCollection(!loadFromAddressables);
            }
...
#if UNITY_EDITOR
        private static ShaderVariantCollection GetDefaultShaderVariantCollection(bool createNewIfNull = true)
        {
            ShaderVariantCollection shaderVariantCollection = GPUInstancerUtility.LoadResource<ShaderVariantCollection>(GPUInstancerConstants.SETTINGS_PATH + GPUInstancerConstants.SHADER_VARIANT_COLLECTION_DEFAULT_NAME);

            if (createNewIfNull && shaderVariantCollection == null)
            {
        ...
        public bool IsBillboardsSupported()
        {
            if (isURP)
            {
                return GPUInstancerUtility.FindShader(GPUInstancerConstants.SHADER_GPUI_BILLBOARD_2D_RENDERER_URP) != null;
            }
            else if (isHDRP)
            {
                return GPUInstancerUtility.FindShader(GPUInstancerConstants.SHADER_GPUI_BILLBOARD_2D_RENDERER_HDRP) != null;
            }
            return true;
        }
    }
