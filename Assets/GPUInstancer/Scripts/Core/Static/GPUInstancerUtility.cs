    ...
    public static class GPUInstancerUtility
    {
        ...
        public static void GeneratePrototypeBillboard(GPUInstancerPrototype prototype, bool forceRegenerate = false)
        {
            ...
            try
            {
                ...
                Shader billboardAlbedoBakeShader = FindShader(GPUInstancerConstants.SHADER_GPUI_BILLBOARD_ALBEDO_BAKER);
                Shader billboardNormalBakeShader = FindShader(GPUInstancerConstants.SHADER_GPUI_BILLBOARD_NORMAL_BAKER);
        ...
        {
            ComputeShader dilationCompute = LoadResource<ComputeShader>(GPUInstancerConstants.COMPUTE_BILLBOARD_RESOURCE_PATH);
        ...
        public static Material GetBillboardMaterial(GPUInstancerPrototype prototype)
        {
            ...
            {
                billboardMaterial = new Material(FindShader(GPUInstancerConstants.SHADER_GPUI_BILLBOARD_2D_RENDERER_URP));
            }
            else if (GPUInstancerConstants.gpuiSettings.isHDRP)
            {
                billboardMaterial = new Material(FindShader(GPUInstancerConstants.SHADER_GPUI_BILLBOARD_2D_RENDERER_HDRP));

                Material hdrpTemplateMat = LoadResource<Material>(GPUInstancerConstants.BILLBOARD_SHADER_HDRP_TEMPLATE_MATERIAL_PATH);
            ...
            else
            {
                switch (prototype.treeType)
                {
                    ...
                    case GPUInstancerTreeType.SpeedTree8:
                        ...
                        billboardMaterial = new Material(FindShader(GPUInstancerConstants.SHADER_GPUI_BILLBOARD_2D_RENDERER_TREE));
                    ...
                    case GPUInstancerTreeType.TreeCreatorTree:
                        ...
                        billboardMaterial = new Material(FindShader(GPUInstancerConstants.SHADER_GPUI_BILLBOARD_2D_RENDERER_TREECREATOR));
                    ...
                    case GPUInstancerTreeType.SoftOcclusionTree:
                        ...
                        billboardMaterial = new Material(FindShader(GPUInstancerConstants.SHADER_GPUI_BILLBOARD_2D_RENDERER_SOFTOCCLUSION));
                        break;
                }
                ...
                if (billboardMaterial == null)
                {
                    ...
                    for (int r = 0; r < prototypeMRs.Length; r++)
                    {
                        for (int m = 0; m < prototypeMRs[r].sharedMaterials.Length; m++)
                        {
                            ...
                            {
                                billboardMaterial = new Material(FindShader(GPUInstancerConstants.SHADER_GPUI_BILLBOARD_2D_RENDERER_STANDARD));
                                break;
                            }
                        }
                        if (billboardMaterial != null)
        ...
        public static TransformAccessArray ResizeTransformAccessArray(TransformAccessArray previousArray, int newSize, Transform defaultTransform)
        {
...
#if UNITY_6000_0_OR_NEWER
            for (int i = result.length; i < newSize; i++)
                result.Add(defaultTransform);
#else
        ...
        public static List<GameObject> GetCorrespondingPrefabAssetsOfGameObjects(GameObject[] gameObjects)
        {
            ...
            foreach (GameObject go in gameObjects)
            {
                ...
                {
                    GameObject newPrefabRoot = GetCorrespongingPrefabOfVariant(go);
                    if (PrefabUtility.GetPrefabAssetType(newPrefabRoot) == PrefabAssetType.Model)
                        prefabRoot = PrefabUtility.GetCorrespondingObjectFromSource(go);
                    else
