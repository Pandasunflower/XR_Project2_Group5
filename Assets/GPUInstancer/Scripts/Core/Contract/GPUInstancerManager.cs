...
namespace GPUInstancer
{
    public abstract class GPUInstancerManager : MonoBehaviour
    {
                ...
                if (_visibilityComputeShader == null || _instanceVisibilityComputeKernelIDs == null || _cameraComputeKernelIDs == null)
                {
                    switch (GPUInstancerUtility.matrixHandlingType)
                    {
                        case GPUIMatrixHandlingType.MatrixAppend:
                            _visibilityComputeShader = GPUInstancerUtility.LoadResource<ComputeShader>(GPUInstancerConstants.VISIBILITY_COMPUTE_RESOURCE_PATH_VULKAN);
                        ...
                        case GPUIMatrixHandlingType.CopyToTexture:
                            _visibilityComputeShader = GPUInstancerUtility.LoadResource<ComputeShader>(GPUInstancerConstants.VISIBILITY_COMPUTE_RESOURCE_PATH);
                            _bufferToTextureComputeShader = GPUInstancerUtility.LoadResource<ComputeShader>(GPUInstancerConstants.BUFFER_TO_TEXTURE_COMPUTE_RESOURCE_PATH);
                    ...
                    _cameraComputeShader = GPUInstancerUtility.LoadResource<ComputeShader>(GPUInstancerConstants.CAMERA_COMPUTE_RESOURCE_PATH);
                    _cameraComputeShaderVR = GPUInstancerUtility.LoadResource<ComputeShader>(GPUInstancerConstants.CAMERA_VR_COMPUTE_RESOURCE_PATH);
                    ...
                    _argsBufferComputeShader = GPUInstancerUtility.LoadResource<ComputeShader>(GPUInstancerConstants.ARGS_BUFFER_COMPUTE_RESOURCE_PATH);
                ...
                case GPUInstancerTreeType.SpeedTree8:
                    ...
                    if (treePrototype.prefabObject.GetComponent<LODGroup>() != null)
                    {
                        ...
                        for (int lod = 0; lod < speedTreeLODs.Length; lod++)
                        {
                            ...
                            Material[] treeProxyMaterial = new Material[1] { new Material(GPUInstancerUtility.FindShader(GPUInstancerConstants.SHADER_GPUI_TREE_PROXY)) };
        ...
        {
            WindZone[] sceneWindZones = GPUInstancerUtility.FindObjectsOfType<WindZone>();
