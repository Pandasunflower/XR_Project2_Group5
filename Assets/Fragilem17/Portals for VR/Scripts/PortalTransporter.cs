...
namespace Fragilem17.MirrorsAndPortals
{
    ...
    public class PortalTransporter : MonoBehaviour
    {
        ...
        public void ExternalTriggerEnter(GameObject other, bool fromPhysics = false)
        {
            ...
            if (!fromPhysics || (fromPhysics && objPos.z < 0))
            {
                ...
                if (obj && obj.isActiveAndEnabled && !portalableObjects.Contains(obj))
                {
                    ...
                    {
                        // wait one frame to add it
                        //StartCoroutine(AddAfterOneFrame(obj));
                    }*/
        ...
        /*private IEnumerator AddAfterOneFrame(PortalableObject obj)
        {
            yield return null;
            if (!portalableObjects.Contains(obj))
            {
                Debug.Log("added after frame " + obj.gameObject.name);
                portalableObjects.Add(obj);                
            }
        }*/
        ...
        public void ExternalTriggerExit(GameObject other, bool fromPhysics = false)
        {
            ...
            /*if (cloneObject)
            {
                Debug.Log(Portal.name + " gonna exit from portal with this: " + cloneObject.name + " IF " + cloneObjects.Contains(cloneObject) + " fromPhysics: " + fromPhysics);
            }*/
